...
make_helper(rep) {
	...
	if(instr_fetch(eip + 1, 1,SR_CS) == 0xc3) {
		...
		if((ops_decoded.opcode==0xa6||ops_decoded.opcode==0xa7||ops_decoded.opcode==0xae||ops_decoded.opcode==0xaf)&&cpu.ZF==0)
			break;
		}
		len = 1;
	}
...
make_helper(repnz) {
	...
	while(cpu.ecx) {
		...
		/* TODO: Jump out of the while loop if necessary. */
		if((ops_decoded.opcode==0xa6||ops_decoded.opcode==0xa7||ops_decoded.opcode==0xae||ops_decoded.opcode==0xaf)&&cpu.ZF==1)
			break;
	}
